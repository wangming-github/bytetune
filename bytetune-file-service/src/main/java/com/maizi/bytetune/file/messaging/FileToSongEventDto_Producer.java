...
public class FileToSongEventDto_Producer {

    @Value("${bytetune.messaging.topics.file-to-song}")
     ...
     */
    public void publish(FileToSongEventDto event) {

        // 将业务事件封装为通用消息对象
        Message message = Message.builder().key(event.getMd5())       // 使用文件 MD5 作为消息 Key，便于消息分区及后续幂等处理
                .payload(event).build();
     ...
     */
    public void publishBatch(List<FileToSongEventDto> events) {
        ...
        List<Message> messages = events.stream().map(event -> //
                Message.builder().key(event.getMd5())       // 使用文件 MD5 作为消息 Key
                        .payload(event).build()).toList();
